...
def generate_max_and_num_attempts(count=100):
    for _ in xrange(count):
        max_attempts = random.randint(1, 100)
        num_attempts = random.randint(0, 100)
        expect_validation_error = max_attempts <= num_attempts
        yield max_attempts, num_attempts, expect_validation_error
